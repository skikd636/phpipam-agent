...
*/
    ...
    private function reset_dhcp_addresses () {
        ...
        $searchdate = $this->now - intval($statuses['1']);

        # Get all stale DHCP addresses
        $query = "SELECT `ip_addr`, `subnetId`, `lastSeen` FROM `ipaddresses` WHERE `state` = ? AND `lastSeen` < ? AND `lastSeen` > ?;";
        $vars = array("4", date('Y-m-d H:i:s', $searchdate), "2000-01-01 00:00:00");
        // fetch details
        ...
        # Get reset hostnames for DHCP addresses 
        $query = "SELECT `ip_addr`, `subnetId`, `lastSeen`, `hostname` FROM `ipaddresses` WHERE `state` = ? AND `lastSeen` > ?;";
        $vars = array("4", "2000-01-01 00:00:00");
        // fetch details
        try { $DHCPAddresses = $this->Database->getObjectsQuery($query, $vars); }
        catch (Exception $e) {
        $this->Result->throw_exception (500, "Error: ".$e->getMessage());
        ...
        foreach ($DHCPAddresses as $addr) {
            $tmp = new stdClass();
            $tmp->ip_addr = $addr['ip_addr'];
            $hostname = $this->resolve_address($tmp, true);
            if ( $hostname['name'] != $addr["hostname"] ) {
