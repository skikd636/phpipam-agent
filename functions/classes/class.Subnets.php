...
class Subnets extends Common_functions {
	...
	*/
	...
	public function find_inactive_hosts ($timelimit = 86400, $limit = 100) {
    	...
    	$this->get_settings ();
    	// search
 		try { $res = $this->Database->getObjectsQuery("select ipaddresses.* from `ipaddresses` join subnets on ipaddresses.subnetId = subnets.id where subnets.pingSubnet = 1 and `lastSeen` between ? and ? order by lastSeen desc limit $limit;", array(gmdate("Y-m-d H:i:s", strtotime(gmdate("Y-m-d H:i:s"))-$timelimit), gmdate("Y-m-d H:i:s", strtotime(gmdate("Y-m-d H:i:s"))-(int) str_replace(";","",strstr($this->settings->pingStatus, ";")))) ); }
		catch (Exception $e) {
			$this->Result->show("danger", _("Error: ").$e->getMessage());
			return false;
		}
